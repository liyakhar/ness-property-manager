...
export function LayoutControls(props: LayoutControlsProps) {
  ...
  const handleValueChange = async (key: string, value: string) => {
    ...
    if (key === 'content_layout') {
      const contentLayout = value as ContentLayout;
      updateContentLayout(contentLayout as 'centered' | 'full-width');
    }

    if (key === 'sidebar_variant') {
      // Handle sidebar variant if needed
    }

    if (key === 'sidebar_collapsible') {
      // Handle sidebar collapsible if needed
    }
